...
async function getDogs(race) {
  ...
  if (race) {
    let raceFinished = [];
    if (race.includes(" ")) {
      let arrWord = race.split(" ");
      arrWord.forEach((e) => {
        let raceToLower = e.toLowerCase();
        let raceFixed =
          raceToLower.charAt(0).toUpperCase() + raceToLower.slice(1);
        raceFinished.push(raceFixed);
      });
    } else {
      let raceToLower = race.toLowerCase();
      let raceFixed =
        raceToLower.charAt(0).toUpperCase() + raceToLower.slice(1);
      raceFinished.push(raceFixed);
    }
    ...
    allDogs.forEach((e) => {
      ...
      if (e.weight.metric) {
        ...
        let temperArr = e.tempers.map(
          (e) => e.name.charAt(0).toUpperCase() + e.name.slice(1)
        );
        temperaments = temperArr.join();
      }

      for (let i = 0; i < raceFinished.length; i++) {
        if (e.name.includes(raceFinished[i])) {
          filterDogs.push({
            name: e.name,
            image: e.image,
...
async function getDogById(id) {
  ...
  if (index !== -1) {
    ...
    if (!allDogs[index].weight.metric) {
      let temperaments;
      let temperArr = allDogs[index].tempers.map(
        (e) => e.name.charAt(0).toUpperCase() + e.name.slice(1)
      );
      ...
      let obj = {
        ...
        image:
          "https://previews.123rf.com/images/cundrawan703/cundrawan7031207/cundrawan703120700008/14519717-perro-avatar-dibujos-animados-icono-de-car%C3%A1cter.jpg",
        temperament: temperaments,
      ...
      obj = {
        name: allDogs[index].name,
        image: allDogs[index].image.url
          ? allDogs[index].image.url
          : "dog hasn't image",
